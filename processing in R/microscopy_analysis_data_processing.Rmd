---
title: "Processing of microscopy analysis results in R"
...
```{r "Create a list ofrequired packages"}
# the following packages are required for:
# 'data.table' - work with data tables (enhanced version of the data.frame)
...
Scripts for plotting in `R` are provided in this `Markdown` as well. Here, also more complex plots are available, where all raw data can be plotted for each biological replicate to create `SuperPlots` and `Pirateplots`.

## Load data from the `Results tablev` file into a data table and look inside
...
Extract channel number from file name. From the column names, we extract the names of those that describe the data files, including the biological replicate date (all columns between the `exp_code` and `frame` columns) and of those that contain the results of the analysis (all columns after `frame`):

```{r "Load the 'Results' table"}
# channel_number <- sub(".*channel ([0-9]+).*", "\\1", results_file)
# in the following code, the regexpr() function returns position and length of the substring, the regmatches() function extracts the actual substring
channel <- regmatches(results_file, regexpr("channel [0-9]", results_file))
if (length(channel) == 0){
  channel_no <- dlg_input(message = "Input channel number for which analysis was performed:", default = "1", gui = .GUI)$res
  channel <- glue("channel {channel_no}")
}
  
data_table <- data.table(read.csv(file = results_file, header = TRUE, sep = ",", quote = "", dec = ".", comment.char = "#"))

# Rename 'control' to '_control'; this way the control sample appears first (leftmost) in the graphs and analyses
for (col in names(data_table))
  set(data_table, i = which(data_table[[col]] == "control"), j = col, value = "_control")
```

```{r "Sort columns into descriptive and quantified parameters"}
# Find the position of the 'frame' column
# This column separates descriptive data (on the left) from measurement data (on the right)
frame_column <- which(colnames(data_table)=="frame")

# Get total number of columns (i.e., position of the last column)
last_column <- ncol(data_table)

# Get names of descriptive columns (used to identify unique samples)
# Skip the first two columns: the first has experiment codes, the second has replicate dates
parameters <- colnames(data_table)[3:(frame_column-1)]

# Get names of columns with measurement data (used to calculate means and SDs)
columns <- colnames(data_table)[(frame_column+1):last_column]
```
...
```{r "Inform user of the available parameters", echo = FALSE, collapse = TRUE}
cat("Results table to be processed:\n")
results_file
cat("\nExperiment code(s):\n")
unique(data_table$exp_code)
cat("\nAvailable descriptive parameters:\n")
parameters
cat("\nAvailable quantification parameters:\n")
columns
```
...
for (EXP in unique(data_table$exp_code)){
  ...
  if (!file.exists(pdf_dir))
    dir.create(pdf_dir)
  violin_dir <- file.path(workingDir, glue("analysis/{EXP}/{channel}/violin_plots"))
  if (!file.exists(violin_dir))
    dir.create(violin_dir)
  tables_dir <- file.path(workingDir, glue("analysis/{EXP}/{channel}/summary_tables"))
  if (!file.exists(tables_dir))
    dir.create(tables_dir)
  stats_dir <- file.path(workingDir, glue("analysis/{EXP}/{channel}/statistics"))
...
Before proceeding, we need to make sure that the `cultivation times` are treated as categories/strings/text, not as numbers. 

```{r}
if ("time" %in% colnames(data_table))
  data_table$time = as.character(data_table$time)
```

Now we can filter and make graphs.

```{r "Results table filtering and summarization"}
for (EXP in unique(data_table$exp_code)){
...
#  writexl::write_xlsx(as.data.frame(summary), path = here("", out_file))
  ...
  for (COL in columns_mean){
    ...
    out_file <- glue("/analysis/{EXP}/{channel}/summary_tables/{COL}.xlsx")
#    out_file <- glue("{tables_dir}/{COL}.xlsx")
...
#  summary$strain <- factor(summary$strain, levels=c("WT", "pil1D", "lsp1D", "lsp1Dpil1D"))
...
#      figure_file <- here(glue("analysis/{EXP}/{channel}/{GRAPH}_{FACET}.png"))
      figure_file <- glue("{workingDir}/analysis/{EXP}/{channel}/violin_plots/{GRAPH}_{FACET}.png") 
...
#        out_file_stat <- glue("analysis/{EXP}/{channel}/{COL}_statistical_analysis.txt")
        out_file_stat <- glue("{workingDir}/analysis/{EXP}/{channel}/statistics/{COL}_statistical_analysis.txt")
