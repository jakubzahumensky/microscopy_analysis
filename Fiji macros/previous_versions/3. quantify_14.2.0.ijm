 ...
 *  These include: area (size), mean and intergal fluorescence intensity of the whole ROI,
 *  its circumference (i.e., plasma membrane if a ROI corresponds to the cell), its inside,
 *  number of high-intensity foci (microdomains) in the plasma membrane, etc.
 *  
 * Additional plugins required:
 *  Adjustable_Watershed - https:imagej.net/plugins/adjustable-watershed/adjustable-watershed
 *  -> download the .class file to the plugins folder)
...
image_types = newArray("tangential", "transversal"); // there are either tranversal (going through the middle) or tangential (showing the surface) microscopy images. Z-stack projections are a special case of the latter.
...
cell_size_min = 0; // by default, cells with area smaller than 5 um^2 are excluded from the analysis. Can be changed in the dialog window below when analysis is run
 ...
 *
 * Display the initial dialog window that prompts the user to specify the folder and image type to be processed/analyzed.
 * The window includes a help message. Multiple parameters need to be set by the user.
 * Detailed explanation in the help message (and published protocol)
 */
help_message = "<html>"
	...
	+ "Only cells within this range will be included in the analysis (this does not change the saved RoiSets). The default lower limit is set to 5 &micro;m<sup>2</sup>, "
	+ "which corresponds to a small bud of a haploid yeast. "
...
Dialog.create("Quantify");
	...
	Dialog.addDirectory("Directory:", "D:/Yeast/EXPERIMENTAL/microscopy/PV-M-250306 - Autodot/tangential/");
	...
	Dialog.addCheckbox("Save segmentation masks", true);
...
function analyze_transversal(){
	...
	init = 0; // first ROI
	// a shortened loop that can be used for testing; activated in the beginning of the script by assigning: test = true; only last 4 ROIs are analyzed	
	if (test == true)
		init = numROIs - 4;
...
//			run("Enlarge...", "enlarge=-" + Delta);
			...
			foci_find_maxima = 0; // initial value
			run("Clear Results");
			...
			foci_find_maxima = number_of_foci;
...
//				run("Enlarge...", "enlarge=-"+Delta);
				...
				cell_background = measure_cell_background();
				run("Clear Results");
				run("Measure");
				DUP_cell_max = getResult("Max", 0);
				cell_area = getResult("Area", 0);
				setThreshold(cell_background[0] + 0.2*(DUP_cell_max - cell_background[1]), pow(2,bit_depth) - 1);
				run("Create Selection");
				foci_area = measure_area_selection();
				foci_area_fraction_JaM = 100*foci_area[0]/cell_area;
...
//				run("Enlarge...", "enlarge=-"+Delta);
				...
				if (mask_mean > 0){
					...
					run("Clear Results");
//					run("Analyze Particles...", "size=" + 5*pow(pixelHeight,2) + "-" + 120*pow(pixelHeight,2) + " show=Nothing display exclude clear stack"); 
//					run("Despeckle");
					run("Analyze Particles...", "size=" + 5*pow(pixelHeight,2) + "-" + 120*pow(pixelHeight,2) + " show=Masks display exclude clear stack");
//					run("Invert");
//					run("Dilate");
//					run("Invert");
		
					if (SaveMasks == true) {
						saveAs("PNG", PatchesDir+title+"-"+j+1);
						rename("MASK");
					}
					number_of_foci = nResults;
					foci_analyze_particles = number_of_foci;
...
//					selectWindow("mask");
			...
			cell_res = parents[0] +","+ parents[1]
				...
				+","+ foci_density_find_maxima +","+ foci_density_analyze_particles +","+ foci_find_maxima +","+ foci_analyze_particles +","+ foci_area_fraction +","+ foci_area_fraction_JaM
...
function print_header(){
	...
	print("["+temp_file+"]","# Coefficient of variance threshold: " + CV_threshold + "\n");
	...
	// in this case, microdomains are always quantified
		...
		column_names = column_names
			+ ",foci_density(find_maxima),foci_density(analyze_particles),foci(find_maxima),foci(analyze_particles),area_fraction(foci_vs_ROI),area_fraction(foci_vs_ROI)-JaM"
