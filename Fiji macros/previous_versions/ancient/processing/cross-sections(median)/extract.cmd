...
find /v "#" Results*.csv | more +2 > Results.tmp
for /f "tokens=1 delims=," %i in (Results.tmp) DO echo %i >> EXP.tmp
sort /unique EXP.tmp > experiments.tmp
del EXP.tmp
...
for x in $(cat experiments.tmp); do \
	...
	find $x Results.tmp | cut -d "," -f 2 | sort | uniq > $x/dates.tmp; \
	find $x Results.tmp | cut -d "," -f 3 | sort | uniq > $x/strains.tmp; \
	find $x Results.tmp | cut -d "," -f 4 | sort | uniq > $x/media.tmp; \
	find $x Results.tmp | cut -d "," -f 5 | sort | uniq > $x/conditions.tmp; \
	for col in $(cat columns.csv); do \
		...
		for DATE in $(cat $x/dates.tmp); do \
			find $x Results.tmp | find ,$MEDIUM, | find ,$STRAIN, | find ,$COND, | find ,$DATE, | awk -v i="$col" -F "," '{a+=$i} END{print a/NR}' &>> $x/$col-$DATE-means.tmp; \
			find $x Results.tmp | find ,$MEDIUM, | find ,$STRAIN, | find ,$COND, | find ,$DATE, | awk -v i="$col" -F "," '{delta = $i - avg; avg += delta / NR; mean2 += delta * ($i - avg); } END { print sqrt(mean2 / NR); }' &>> $x/$col-$DATE-SDs.tmp; \
		done; \
		done; \
		done; \
		done; \
	...
	for MEDIUM in $(cat $x/media.tmp); do \
		...
		for COND in $(cat $x/conditions.tmp); do \
		for DATE in $(cat $x/dates.tmp); do \
			find $x Results.tmp | find ,$MEDIUM, | find ,$STRAIN, | find ,$COND, | find -c ,$DATE, &>> $x/$DATE-cell_count.tmp; \
		done; \
		done; \
		done; \
        done; \
	...
	for col in $(cat columns.csv); do \
		...
		C=$(find -w $col legend.csv | cut -d "," -f 2); \
