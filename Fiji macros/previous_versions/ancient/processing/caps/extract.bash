...
for x in $(cat experiments.tmp); do \
	...
	grep $x Results*.csv | cut -d "," -f 2 | sort | uniq > $x/dates.tmp; \
	grep $x Results*.csv | cut -d "," -f 3 | sort | uniq > $x/strains.tmp; \
	grep $x Results*.csv | cut -d "," -f 4 | sort | uniq > $x/media.tmp; \
	grep $x Results*.csv | cut -d "," -f 6 | sort | uniq > $x/conditions.tmp; \
	for col in $(cat columns.csv); do \
		...
		for DATE in $(cat $x/dates.tmp); do \
			grep $x Results*.csv | grep ,$MEDIUM, | grep ,$STRAIN, | grep ,$COND, | grep ,$DATE, | awk -v i="$col" -F "," '{a+=$i} END{print a/NR}' &>> $x/$col-$DATE-means.tmp; \
			grep $x Results*.csv | grep ,$MEDIUM, | grep ,$STRAIN, | grep ,$COND, | grep ,$DATE, | awk -v i="$col" -F "," '{delta = $i - avg; avg += delta / NR; mean2 += delta * ($i - avg); } END { print sqrt(mean2 / NR); }' &>> $x/$col-$DATE-SDs.tmp; \
		done; \
		done; \
		done; \
		done; \
	...
	for MEDIUM in $(cat $x/media.tmp); do \
		...
		for COND in $(cat $x/conditions.tmp); do \
		for DATE in $(cat $x/dates.tmp); do \
			grep $x Results*.csv | grep ,$MEDIUM, | grep ,$STRAIN, | grep ,$COND, | grep -c ,$DATE, &>> $x/$DATE-cell_count.tmp; \
		done; \
		done; \
		done; \
	done; \
