			...
			if (endsWith(dir, dirType+"/") && indexOf(proc_files, q) < 0 && indexOf(q, subset) >= 0)
				if (check_ROIs(dir, list[i])){
					...
					if (contains(extension_list, ext)){
						...
						print("\\Update: Processing: channel " + ch + "/" + CHANNEL.length-1 + "; file " + counter + "/" + count-proc_files_number);
...
function analyze_transversal(){
	...
	// a shortened loop that can be used for testing; activated in the beginning of the script by assigning: test = 1;
        // only last 4(5?) ROIs are analyzed
	if (test == 1){
		init = numROIs - 4;
	}
