...
// define "dirType" based on what kind of images are to be processed
		...
		// if the list item is a folder, go in and get a list of items
      	if (endsWith(list[i], "/"))
			...
			if (File.exists(file)){ // HOW COULD IT NOT EXIST??? I don't understand this, but I must have put it here for a reason
				...
				if (endsWith(dir, dirType+"/") && indexOf(file, subset) >= 0){
...
function Map_to_ROIs(){
	...
	masksDir = File.getParent(dir)+"/"+replace(File.getName(dir), "data", "masks")+"/";
	if (File.exists(MasksDir + title + MasksSuffix) || File.exists(masksDir + title + MasksSuffix)){
		...
		run("Clear Results");
		run("Measure");
		if (File.exists(MasksDir + title + MasksSuffix))
			open(MasksDir + title + MasksSuffix);
		else
			open(masksDir + title + MasksSuffix);
