...
run("Set Measurements...", "area mean min standard modal bounding redirect=None decimal=5");
countFiles(dir);
processFolder(dir);
...
//	if (endsWith(dir, DirType)) {
		...
		while ((E != 0) || (X != 0) || (Y != 0)) {
			...
			Dialog.addMessage("Stats:\nfolder: \"" + parent_name + "/" + dir_name + "\"" + "\nimage counter: " + i+1 + "/" + list.length + " (" + counter + "/" + count +" total)" + "\nNo. of ROIs: " + numROIs, 14);
			Dialog.addMessage("Adjust all ROIs", 12);
