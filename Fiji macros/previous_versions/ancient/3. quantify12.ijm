...
run("Select None");
*/
//			if (CV_ROI > CV_threshold) //if the CV is not greater than CV_threshold (set to 0.3 by def.), the cells are deemed to have no microdomains
...
function save_temp(){
	...
	saveAs("Text", dirMaster + "Results-temporary(" + image_type + ")");
		processed_file = replace(q, "\\", "/");
	print("[processed_files.tsv]", processed_file+"\n");
