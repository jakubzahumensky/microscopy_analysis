		...
		while ((E != 0) || (X != 0) || (Y != 0)) {
			...
			if (E != 0) {
				...
				for(j = numROIs-1; j >= 0 ; j--) {
					...
					roiManager("Select", j);
					roiManager("Delete");
//					roiManager("update"); does not work here - if I work with microns, it shoudl work
				}
			}
		}
		roiManager("Remove Channel Info");
...
/*
//remove selected ROI if most of its pixels have lower intensity than background + 2*SD
function Clean_ROIs() {
	Measure_BC();
	numROIs = roiManager("count");
	for(j = numROIs-1; j >= 0 ; j--) {
