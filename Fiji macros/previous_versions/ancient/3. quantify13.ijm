...
var results_window_name = "analysis results";
var title="";
var roiDir="";
var patchesDir="";
var pixelHeight = 0;
...
Dialog.create("Quantify");
	Dialog.addDirectory("Directory:", "D:/Yeast/EXPERIMENTAL/microscopy/JZ-M-064-230413 - Candida - test-whole_frame/230413 - test set/");
...
//patches quantified from intensity profiles
			patches_from_intensity_profile_Gauss = patches_from_intensity_profile("DUP_Gauss",1000);
...
//			patch_distribution = charaterize_patch_distribution(patch_numbers[0]); //patch_distance_min, patch_distance_max, patch_distance_mean, patch_distance_stdDev, patch_distance_CV
			patches_from_intensity_profile_CLAHE = patches_from_intensity_profile("DUP_CLAHE",1000);
			patches_from_intensity_profile_dotfind = patches_from_intensity_profile("DUP_dotfind",1000);
...
//			+",prot_in_patches,patch_distance_min,patch_distance_max,patch_distance_mean,patch_distance_stdDev,patch_distance_CV"
		...
		print("exp_code,BR_date," + naming_scheme + ",mean_background,cell_no,patch_density(find_maxima),patch_density(analyze_particles),area_fraction(patch_vs_ROI),length,length_SD,width,width_SD,size,size_SD,mean_patch_intensity,mean_patch_intensity_SD");
	setLocation(0,0);
}
...
*/
//	logString = substring(logString, 0, lengthOf(logString)-2);
//	lastIndexOf(logString, "\n");
//	print("\\Clear");
//	print(logString);
//waitForUser;
...
function patches_from_intensity_profile(win_title,rel_outlier_intensity){
	...
	PM_from_line = measure_PM(j);
	select_ROI(j);
	PM_from_area = measure_area_selection();
//	select_window(win_title);
	cortical_cytosol = measure_cort(j); //array: mean, SD; neither corrected for BC; serves for direct comparison of intensities when PM microdomains are counted
	BC_win_title = measure_background(win_title);
	PM_base = get_PM_base(j);
	PM_base_BC = PM_base - BC_win_title;
//waitForUser(cortical_cytosol[0] - BC+" , "+PM_from_area[2]);
//waitForUser(win_title+" , "+get_PM_base(j)+" , "+PM_base_BC+" , "+BC_win_title+" , "+PatchProminence*PM_base_BC+BC);
//	PM[0]+" , "+PM[1]+" , "+PM_base_BC);
...
//waitForUser(win_title);
	select_window(win_title);
	select_ROI(j);
	run("Area to Line");
	profile = getProfile();

//Plot.create("Detecting Minima and Maxima", "X", "Y", profile);
//Plot.show;
//waitForUser(win_title+" , "+PM_base);
	
	Array.getStatistics(profile, profile_min, profile_max, profile_mean, profile_stdDev);
	maxIndices = Array.findMaxima(profile, profile_stdDev/3, 1);
	maxima = newArray(0);
	M = 0;
	patch_outliers = 0;
	for (jj = 0; jj < maxIndices.length; jj++){
		x = maxIndices[jj];
//		waitForUser(win_title+" , "+profile[x]+" , "+Peak_MIN+", "+(profile[x]-BC)/PM_base_BC);
//waitForUser(profile[x]+" , "+Peak_MIN+" , "+BC+" , "+PM_base_BC);
		if ((profile[x]-BC)/PM_base_BC > rel_outlier_intensity){
			patch_outliers++;
		} else
	...
	patch_intensity_BC = maxima_mean-BC;
...
function patches_from_thresholding(win_title){
	...
	run("Select None");
	PM = measure_PM(j); //PM[0] - length, PM[1] - mean intensity
...
function watershed_segmentation(){
	...
	roiManager("Open", roiDir+title+"-RoiSet.zip");
	roiManager("Remove Channel Info");
	watershedDir = replace(dir, "data", "watershed_segmentation-ch"+ch);
