...
function prep() {
	...
	if (matches(image_type, "transversal")) {
		...
		run("Duplicate...", "title=DUP_profile duplicate channels"+ch);
		run("Duplicate...", "title=DUP_threshold duplicate channels"+ch);
		run("Duplicate...", "title=DUP_watershed duplicate channels"+ch);
		selectWindow(title);
