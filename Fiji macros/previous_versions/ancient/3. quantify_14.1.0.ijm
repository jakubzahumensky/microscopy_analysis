...
//					run("Analyze Particles...", "size=" + 5*pow(pixelHeight,2) + "-" + 120*pow(pixelHeight,2) + " show=Nothing display exclude clear stack"); 
			...
			cell_res = parents[0] +","+ parents[1]
				...
				+","+ foci_density_find_maxima +","+ foci_density_analyze_particles +","+ foci_find_maxima +","+ foci_analyze_particles +","+ foci_area_fraction
...
function print_header(){
	...
	// in this case, microdomains are always quantified
		...
		column_names = column_names
			+ ",foci_density(find_maxima),foci_density(analyze_particles),foci(find_maxima),foci(analyze_particles),area_fraction(foci_vs_ROI)"
