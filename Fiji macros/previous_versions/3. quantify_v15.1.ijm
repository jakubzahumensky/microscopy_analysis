...
version = "15.1"; // not backward compatible with 8.x versions of ROI_prep!
...
for (ii = 0; ii <= CHANNEL.length-1; ii++){
	...
	temp_file_profiles = "results-profiles_temporary_" + image_type + "_channel_"+ ch +".csv";
...
function analyze_transversal(file){
	...
	for (j = init; j < numROIs; j++){
		...
		if (cell[0] > cell_size_min && cell[0] < cell_size_max && cell[4] > CV){
			...
			// save intensity profile along the line delimiting respective ROI
			cell_res_profile = parents[0] + "," + parents[1] // [0] - experiment code, [1] - biological replicate date
				...
				+ "\n";
			print("[" + temp_file_profiles + "]", cell_res_profile);
...
function initialize(continue_analysis){
	...
	if (isOpen(temp_file_profiles))
		print("[" + temp_file_profiles + "]","\\Close");
	...
	setLocation(0,0);
	run("Text Window...", "name=[" + temp_file_profiles + "] width=180 height=40");
	...
	if (continue_analysis == 1 && File.exists(dirMaster + temp_file)){ // if File.exists() - when the analysis is continued and multiple channels are selected, some may not have temporary files
		...
		print("[" + temp_file_profiles + "]", File.openAsString(dirMaster + temp_fil_profiles));
...
function print_header(){
	...
	if (matches(image_type, "transversal")){
		...
		column_names_profiles = "exp_code,BR_date,"
			...
			+ ",pixelWidth"
			+ ",intensity_profile";
		print("[" + temp_file_profiles + "]", column_names_profiles + "\n");
...
function save_temp(){
	...
	selectWindow(temp_file_profiles);
	saveAs("Text", dirMaster + temp_file_profiles);
	setLocation(screenWidth/2, 0);
...
function channel_wrap_up(){
	...
	selectWindow(temp_file_profiles);
	saveAs("Text", dirMaster + res_profiles);
