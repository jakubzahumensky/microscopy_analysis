...
macro_name = "Quantification of cell parameters in microscopy images";
version = "15.2"; // not backward compatible with 8.x versions of ROI_prep!
...
var gauss_sigma = 1; // smoothing radius for Gaussian blur (in pixels)
var foci_prominence = 1.666; // patch prominence threshold (for transversal images) - set semi-empirically
var deconvolved = "no";
...
var naming_scheme = "strain,medium,time,condition,frame";
var experiment_scheme = "XY-M-000";
var subset = "";

var cell_size_min = 5; // by default, cells with area smaller than 5 um^2 are excluded from the analysis. Can be changed in the dialog window below when analysis is run
var cell_size_max = 0;
var CV_threshold = 0;
var quantify_microdomains = false;

var temporary_results_files_count = 0;
var count = 0;
var counter = 1;
...
var temporary_results_file_profiles = "";
var no_ROI_files = "";
var already_processed_files_number = 0;
var already_processed_files = "";
...
get_user_input(initial_folder);
CHANNEL = sort_channels(channel_input);
adjust_for_deconvolved(deconvolved);
count_files(dir_master);
check_temporary(CHANNEL);
process_channels();
final_wrap_up();
 ...
 * Display the "Quantify" dialog window, including a help message. Multiple parameters need to be set by the user.
 * Detailed explanation in the help message (and protocol)
 */
function get_user_input(initial_folder){
	help_message = "<html>"
		...
		+ "<i>The user is advised to measure a handful of cells before adjusting these limits. If in doubt, set limits 0-Infinity and filter the results table.</i> <br><br>"
		...
		+ "but <i>a priori</i> knowledge about the system is required. Filtering by CV can be performed <i>ex post</i> by filtering the results table. <br><br>"

		+ "<b>Quantify microdomains</b><br>"
		+ "Select if you wish to quantify plasma membrane microdomains: number per cell image, density, mean intensity etc. The analysis run is much shorter when microdomains are not being analyzed. <br><br>"
		...
		+ "Proceed with caution and verify that the results make sense. <br><br>"
		+ "</html>";

	Dialog.create("Quantify");
		Dialog.addMessage("Note: Images with following extensions are processed: " + String.join(extension_list) +". \nIf your files have another extension, please add it to the 'extension_list' array in line 52.");
		Dialog.addDirectory("Directory:", initial_folder);
		Dialog.addString("Subset (optional):", subset);
		Dialog.addString("Channel(s):", ch);
		Dialog.addString("Naming scheme:", naming_scheme, 33);
		Dialog.addString("Experiment code scheme:", experiment_scheme, 33);
	  ...
	  Dialog.show();
		dir_master = fix_folder_input(Dialog.getString());
		subset = Dialog.getString();
		channel_input = replace(Dialog.getString(), " ", "");
		...
		quantify_microdomains = Dialog.getChoice();
 ...
 */
function fix_folder_input(folder_input){
...
/* Sort the user-input to create an array containing the selected channels (individual number, comma-separated list, range) to be processed */
function sort_channels(channel_string){
	if (channel_string <= 0)
		exit("Selected channel (" + channel_string + ") does not exit.");
	if (indexOf(channel_string, "-") >= 0){
		X = "--";
		channel_temp = split(channel_string,"--");
...
function count_files(dir){
	...
	for (i = 0; i < list.length; i++){
		if (endsWith(list[i], "/"))
			...
			file = dir + list[i];
			if (endsWith(dir, image_type + "/data/") && indexOf(already_processed_files, file) < 0 && indexOf(file, subset) >= 0){
				count++;
				...
				if (channels < CHANNEL[CHANNEL.length-1])
					exit("One or more images in the data set do not have one or more selected channels (" + channel_input + "). Check your data and restart analysis.");
				close();
			}
		}
	}
...
function check_temporary(CHANNEL){
	for (ii = 0; ii <= CHANNEL.length - 1; ii++){
		ch = CHANNEL[ii];
		temporary_results_file = "results-temporary_" + image_type + "_channel_"+ ch +".csv";
		if (array_contains(getFileList(dir_master), temporary_results_file))
			temporary_results_files_count++;
	}

	if (temporary_results_files_count > 0)
 ...
 * process the images in a channel-wise manner, i.e., process all images for one channel, then for the other etc. (based on use input)
 */
function process_channels(){
	for (ii = 0; ii <= CHANNEL.length-1; ii++){
		ch = CHANNEL[ii];
		counter = 1; // resets the counter for the current channel
		prepare_file_names();
		initialize(continue_analysis);
		process_folder(dir_master);
		channel_wrap_up();
	}
}
...
/****************************************************************************************************************************************************/
/* PREPARATORY STEPS FOR THE ANALYSIS OF THE CURRENT CHANNEL - FILENAMES, FILES, HEADER, COLUMNS

/* assign values to file name variables based on the current channal and image type */
function prepare_file_names(){
	temporary_results_file = "results-temporary_" + image_type + "_channel_"+ ch +".csv";
	temporary_results_file_profiles = "results-profiles_temporary_" + image_type + "_channel_"+ ch +".csv";
	no_ROI_files = "files_without_ROIs_" + image_type + "_channel_"+ ch +".tsv";
	processed_images_file = "processed_images_file_" + image_type + "_channel_"+ ch +".tsv";
}
...
function initialize(continue_analysis){
	...
	if (continue_analysis == 1 && File.exists(dir_master + temporary_results_file)){ // if File.exists() - when the analysis is continued and multiple channels are selected, some may not have temporary files
		if (File.exists(dir_master + no_ROI_files)){
			File.delete(dir_master + no_ROI_files);
			close("Log");
		}
		print("[" + temporary_results_file + "]", File.openAsString(dir_master + temporary_results_file));
		print("[" + temporary_results_file_profiles + "]", File.openAsString(dir_master + temporary_results_file_profiles));
		already_processed_files = File.openAsString(dir_master + processed_images_file);
		print("[" + processed_images_file + "]", already_processed_files);
		already_processed_files_array = split(already_processed_files,"\n");
		already_processed_files_number = already_processed_files_array.length;
	} else {
		print_header();
		print_column_names();
	}
...
/* close specific text windows that might be open
 * create new text windows to write temporary results, names of processed files and files without defined ROIs
 */
function prepare_text_windows(){
	windows_list = newArray("Log", temporary_results_file, temporary_results_file_profiles, processed_images_file, no_ROI_files);
	close_windows(windows_list);
	run("Text Window...", "name=[" + temporary_results_file + "] width=180 height=40");
	setLocation(0,0);
	run("Text Window...", "name=[" + temporary_results_file_profiles + "] width=180 height=40");
	setLocation(screenWidth/2,0);
	run("Text Window...", "name=[" + processed_images_file + "] width=180 height=20");
...
function print_header(){
	...
	print("[" + temporary_results_file + "]","# Channel: " + ch + "\n");
	print("[" + temporary_results_file + "]","# Cell (ROI) size interval: " + cell_size_min + "-" + cell_size_max +" um^2"+ "\n");
	print("[" + temporary_results_file + "]","# Coefficient of variance threshold: " + CV_threshold + "\n");
	if (matches(image_type, "transversal")){
		print("[" + temporary_results_file + "]","# Smoothing radius (Gaussian blur): " + gauss_sigma + "\n");
		print("[" + temporary_results_file + "]","# Patch prominence: " + foci_prominence + "\n");
	}
...
function print_column_names(){
	...
	if (matches(image_type, "transversal")){
		column_names = column_names
			...
			+ ",plasma_membrane_I.div.cytosol_I(mean),plasma_membrane_I.div.cell_I(integrated),cytosol_I.div.cell_I(integrated)";
		...
		column_names_profiles = "exp_code,BR_date,"
			...
			+ ",pixelWidth"
			+ ",intensity_profile";
		print("[" + temporary_results_file_profiles + "]", column_names_profiles + "\n");
...
/****************************************************************************************************************************************************/
/* BASIC STRUCTURE FOR RECURSIVE DATA PROCESSING - WITHIN THE ANALYSIS OF THE CURRENT CHANNEL
 *
 * The overal logic is that each image/file is first opened and then the process is selected.
 * The following function makes a list of contents of specified folder (folders and files) and goes through it one by one.
 ...
 */
function image_has_ROIs(dir, string){
	title = substring(string, 0, lastIndexOf(string, "."));
	core_title = clean_title(title);
	ROI_dir = File.getParent(dir) + "/" + replace(File.getName(dir), "data", "ROIs") + "/";
	if (File.exists(ROI_dir + core_title + RoiSet_suffix)){
		return true;
	} else {
		print("[" + no_ROI_files + "]", file + "\n");
		selectWindow(no_ROI_files);
		saveAs("Text", dir_master + no_ROI_files);
		return false;
	}
}
...
function print_status(){
	print("\\Clear");
	print("Processing:");
	print("channel: " + ch + "; user channel input: " + channel_input);
	print("file: " + counter + "/" + count-already_processed_files_number);
...
function estimate_remaining_time(ii){
	...
	files_left_current_channel = (count - already_processed_files_number - counter);
...
function analyze_transversal(file){
	...
	for (j = init; j < numROIs; j++){
		...
		if (cell[0] > cell_size_min && cell[0] < cell_size_max && cell[4] > CV_threshold){
			// preparation for plasma membrane segmentation
			run("Create Mask"); // creates a mask of the entire cell from the ROI
			rename("Mask-cell");
			
			/* cytosol segmentation and measurement */
			...
			run("Create Mask"); // creates a cytosol mask from ROI
			rename("Mask-cytosol");
			
			/* plasma membrane segmentation and measurement */
			...
			internal_foci_by_thresholding = measure_internal_foci_by_thresholding();
			...
			print_results_transversal();
			windows_list = newArray("Mask-cell", "Mask-cytosol", "Mask-cyt-outer", "Mask-cyt-inner", "Result of Mask-cell", "Result of Mask-cyt-outer");
			close_windows(windows_list);
		}
	}
...
function measure_internal_foci_by_thresholding(){
	...
	run("Select None");
	setThreshold(ROI_mean + 2*ROI_std, 65535, "raw");
	run("Create Mask");
	...
	windows_list = newArray("DUP_cell", foci_mask, "Mask of " + foci_mask, "DUP_cell_corr");
	close_windows(windows_list);
...
function count_foci_from_intensity_profile(window_title,relative_outlier_intensity_threshold){
	...
	cortical_cytosol = measure_cortex(window_title, j); // array: mean, SD; neither corrected for background; serves for direct comparison of intensities when plasma_membrane microdomains are counted
 ...
 */
function measure_cortex(img_title, ROI_no){
 ...
 * The obtained value is similar to that obtained from plasma membrane segmentation, but different, as the line has a small gap between end and beginning.
 */
...
/* save results of transversal data analysis */
function print_results_transversal(){
	...
	cell_res = cell_res
		+ "," + String.join(internal_foci_by_thresholding);
	cell_res = cell_res + "\n";
	print("[" + temporary_results_file + "]", cell_res);

	// intensity profile along the line delimiting respective ROI
	cell_res_profile = parents[0] + "," + parents[1] // [0] - experiment code, [1] - biological replicate date
		+ "," + replace(title," ","_") + "," + background + "," + (j+1)
		+ "," + pixelWidth
		+ "," + replace(String.join(profile)," ","")
		+ "\n";
	print("[" + temporary_results_file_profiles + "]", cell_res_profile);
}



/****************************************************************************************************************************************************/
/* ANALYZE TANGENTIAL IMAGES */
function analyze_tangential(file){
	...
	for(j = init; j < numROIs; j++){
		...
		if (cell[0] > cell_size_min && cell[0] < cell_size_max){ // continue if the cell size falls between the lower and upper limit specified by the user when the macro is run
			...
			if (cell[4] > CV_threshold) // [4] - cell intensity CV; if the CV is not greater than CV (set by the user in the initial Dialog window), the cells are deemed to have no microdomains
...
//				run("Enlarge...", "enlarge=-"+Delta);
				...
				windows_list = newArray("mask", "DUP_cell", "DUP_cell_CLAHE");
				close_windows(windows_list);
			}
			print_results_tangential();
		}
	}
	close("*");
	save_temp();
...
function prepare(file){
	core_title = prepare_image(file);
	prepare_ROI_manager();
	preprocess_gaussian_blur();
	...
	preprocess_clahe();
	// The following are only prepared for transversal images and omly if the 'quantify microdomains' is selected
	if (matches(image_type, "transversal") && quantify_microdomains == "yes"){
		preprocess_mean_smoothing();
		preprocess_dotfind("DUP_mean");
		watershed_segmentation(core_title);
	}
}


/* Open image file and remove all suffixes that may be there (AVG, SUM, etc.).
 * This should result in the core file name, as it was originally saved after imaging.
 */
function prepare_image(img_file){
	run("Bio-Formats (Windowless)", "open=[" + img_file + "]");
	rename(list[i]);
 ...
 * Note that the switch to this approach makes this version incompatible with previous versions of 8.x and lower and also incompatible with Quatify version 14.x and lower!
 */
...
/* Clear ROI manager, open ROI Set corresponding to the current image, if it exists.
 * Remove information about channels before saving the current ROIs - the same ROIs are used for all available channels.
 */
function prepare_ROI_manager(){
	ROI_dir = File.getParent(dir) + "/" + replace(File.getName(dir), "data", "ROIs") + "/";
	roiManager("reset");
	roiManager("Open", ROI_dir + core_title + RoiSet_suffix);
...
/* basic Gaussian smoothing used for all images */
function preprocess_gaussian_blur(){
	selectWindow(list[i]);
...
/* local contrast adjustments to bring out local high-intensity foci */
function preprocess_clahe(){
	selectWindow(list[i]);
...
function preprocess_mean_smoothing(){
	selectWindow(list[i]);
...
function measure_area_selection(){
	...
	area_SD = getResult("StdDev", 0); // standard deviation of the mean intensity
	area_CV = area_SD/mean_intensity_background;
	...
	return newArray(area, integrated_intensity_background, mean_intensity_background, area_SD, area_CV, axis_major, axis_minor, eccentricity);
}
...
function save_temp(){
	...
	selectWindow(temporary_results_file_profiles);
	saveAs("Text", dir_master + temporary_results_file_profiles);
	setLocation(screenWidth/2, 0);

	print("[" + processed_images_file + "]", file + "\n");
	selectWindow(processed_images_file);
	saveAs("Text", dir_master + processed_images_file);
}


/****************************************************************************************************************************************************/
/* WRAP-UP AFTER ANALYSIS OF THE CURRENT CHANNEL
...
function channel_wrap_up(){
	...
	res_profiles = "Intensity profiles from " + image_type + " image analysis, channel " + ch + " (" + year + "-" + String.pad(month + 1,2) + "-" + String.pad(dayOfMonth,2) + "," + String.pad(hour,2) + "-" + String.pad(minute,2) + "-" + String.pad(second,2) + ").csv";
	selectWindow(temporary_results_file_profiles);
	saveAs("Text", dir_master + res_profiles);

	if (File.length(dir_master + no_ROI_files) == 0){
		File.delete(dir_master + temporary_results_file);
		File.delete(dir_master + temporary_results_file_profiles);
	}
	
	windows_list = newArray("Log", "Results", "ROI manager", processed_images_file, no_ROI_files, res, res_profiles, "Exception");
	close_windows(windows_list);
}
...
function final_wrap_up(){
	...
	processed_images_file_count = 0;
	no_ROI_files_count = 0;
	for (ii = 0; ii <= CHANNEL.length-1; ii++){
		ch = CHANNEL[ii];
		processed_images_file = "processed_images_file_" + image_type + "_channel_"+ ch +".tsv";
		no_ROI_files = "files_without_ROIs_" + image_type + "_channel_"+ ch +".tsv";
		if (File.length(dir_master + processed_images_file) > 0)
			processed_images_file_count++;
		if (File.length(dir_master + no_ROI_files) > 0)
			no_ROI_files_count++;
	}
	if (processed_images_file_count == 0)
