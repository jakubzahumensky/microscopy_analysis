...
//mask_suffix = newArray("_cp_masks.png", "_cp_masks.tif");
...
Dialog.create("Select experiment directory, process and image type");
	Dialog.addMessage("Note: ROIs for images with following extensions are processed: " + extlist +". \nIf your files have another extension, please add it to the 'extension_list' array in line 44.");
...
if (isOpen("Log") && RoiSet_count > 0){
	...
	missing_RoiSet_count = count_RoiSet_files(dir, false); // count files that DON NOT HAVE defines ROIs (i.e., option FALSE is used)
	if (missing_RoiSet_count > 0)
		if (getBoolean("WARNING!\n" + missing_RoiSet_count + " images do not have defined sets of ROIs (listed in the Log window).\nDo you wish to continue?") == 0)
			...
			define_manually = getBoolean("Do you want to define missing ROIs manually?");
		...
		if (endsWith(list[i], "/")) // if the list item is a folder, go in and get a list of items
			...
			if (endsWith(dir, image_type + "/data/") && indexOf(file, subset) >= 0){
				...
				if (contains(extension_list, ext)){
					...
					// if conversion of masks to ROIs is selected, perform it; this runs in batch mode, i.e., no images are shown to the user
					// to enable progress monitoring, a status window is shown with a file counter
					if (matches(process, "Convert Masks to ROIs")){
						// run the following either if 'overwrite' was selected or if it wasn't and the respective file has not been processed yet
...
function count_RoiSet_files(dir, boo){
	list = getFileList(dir);
	for (i = 0; i < list.length; i++){
		if (endsWith(list[i], "/"))
			count_RoiSet_files("" + dir + list[i], boo);
		else {
			file = dir + list[i];
			if (endsWith(dir, image_type + "/data/") && indexOf(file, subset) >= 0){
				extIndex = lastIndexOf(file, ".");
				...
				if (contains(extension_list, ext)){
					...
					if (File.exists(roiDir + core_title + RoiSet_suffix) == boo){
						RS_count++;
						print(file);
					}
				}
...
function contains(array, value){
    for (i = 0; i < array.length; i++)
        if (array[i] == value)
        	return true;
/*
	filtered = Array.filter(extension_list, "xxx")
...
function prepare(file){
	...
	if (channels > 1)
   	...
   	rename(list[i]);
	dummy_name = i; // when experimenter blinding is used, the current counter value is displayed instead of the image title
	roiDir = File.getParent(dir) + "/" + replace(File.getName(dir), "data", "ROIs") + "/";
...
function map_to_ROIs(file){
	// the segmentation masks may be stored in a dir called "Masks" or "masks"; the macro can find either; any other directory name gives an error
	...
	// When counting backwards, these will have already been processed when changes are made.
		if (numROIs > 0){
			for (j = numROIs-1; j >= 0 ; j--){
				...
				too_irregular = (ROI_circularity < 0.75);
			...
			// remove information about the ROIs that would tie them to a specific channel/slice/time frame
			// This way, same ROIs can be used in all channels, slices, etc.
			roiManager("Remove Channel Info");
	        roiManager("Remove Slice Info");
			roiManager("Remove Frame Info");
			numROIs = roiManager("count");
		}
		// if there are no ROIs in the ROI manager, create one of the whole image.
		// This is required, because if no ROIs are in the ROI manager, the macro crashes, as there would be nothing to save in the subsequent step
		if (numROIs == 0){
			makeRectangle(0, 0, 100, 100);
			roiManager("Add");
		}
//		roiManager("Save", roiDir + title + RoiSet_suffix); // save the updated list of ROIs
		roiManager("Save", roiDir + core_title + RoiSet_suffix); // save the updated list of ROIs
		close("*");
		counter++;
	} else
...
// function ROI_check() opens each raw file in the data folder and loads the respective ROIs (RoiSet) made by the map_to_ROIs() function.
// It then allows the user to check the existing ROIs, adjust/remove them, or add additional ones.
// Option to enlarge/shrink size of all ROIs is implemented. For shrinking input negative values.
function ROI_check(file, k){
	...
	core_title = prepare(file);
	roiDir = File.getParent(dir) + "/" + replace(File.getName(dir), "data", "ROIs") + "/";
//	roiFile = roiDir + title + RoiSet_suffix;
	roiFile = roiDir + core_title + RoiSet_suffix;
//	if (File.exists(roiDir + title + RoiSet_suffix)){
	...
	// Create a non-blocking dialog window that allows the user to:
	// move the ROIs (all at the same time) in all directions, make them bigger/smaller, jump forward/backwards and exclude the current image from analysis.
	...
	html = "<html>"
		...
		+"To add a new ROI, first make a selection (the elliptical tool is preselected) and then press the \"<i>Add</i>\" button in the ROI manager window or the \"<i>t</i>\" key on the keyboard.<br>"
		...
		+"Entering a non-zero value in any of the following fields changes all of the ROIs and displays them for assessment.<br>"
		...
		+"&#8226; <i>Move right</i> - move all ROIs to the right by a specified number of pixels (negative values move the ROIs to the left).<br>"
		+"&#8226; <i>Move down</i> - move all ROIs down by a specified number of pixels (negative values move the ROIs up).<br>"
		+"<br>"
		+"<b>Remove all ROIs with area smaller than selected value</b><br>"
		+"All ROIs with area smaller than the input value (in &micro;m<sup>2</sup>) are automatically removed from the ROI manager (and in turn from the RoiSet).<br>"
		+"<br>"
		...
		+"If selected, the current image is moved to the <i>data-exclude</i> folder and is excluded from the analysis when run. Can be used e.g. for incorrectly focused images.<br>"
		+"<br>"
		+"<b>Note on ROI placement</b><br>"
		+html_ROIs;
	size_change = 1; // the initial size change is set to a non-zero value so that the dialog window below is shown ("while" condition is used)
	shift_x = 0;
	shift_y = 0;
	size_threshold = 0;
	...
	roiManager("Save", roiDir + core_title + RoiSet_suffix);
