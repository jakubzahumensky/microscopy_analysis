...
//
// if you use this script, please cite at least the first two of the following publications:
...
// BioProtocol is currently in preparation - look for it in BioRxiv and then later at the BioProtocols website
//
// Abbreviations used:
// cyt - cytosol
...
Dialog.create("Quantify");
	Dialog.addDirectory("Directory:", "D:/Yeast/EXPERIMENTAL/microscopy/JZ-M-066-231002 - Sur7 vs C-term modifications/231004/transversal/");
...
function processFolder(dir){
	...
	for (i=0; i<list.length; i++){
	 ...
	 // if the item is a file, get its name with the whole path
			...
			if (endsWith(dir, image_type + "/data/") && indexOf(proc_files, q) < 0 && indexOf(q, subset) >= 0)
...
function analyze_transversal(){
	...
	prepare(); // bit-depth and pixel size of the image are read, ROI-Set is opened, CLAHE-processed image is made
	...
	end_ROI = numROIs;
// a shortened loop that can be used for testing; activated in the beginning of the script by assigning: test = 1; only first 5 ROIs are analyzed
	if (test == 1)
		end_ROI = 5;
	...
	for (j = 0; j < end_ROI; j++){
		...
		// [0] - area, [1] - integral_intensity_background, [2] - mean_intensity_background, [3] - SD, [4] - CV, [5] - major axis, [6] - minor axis, [7] - eccentricity
		...
		if (cell[0] > cell_size_min && cell[0] < cell_size_max && cell[4] > CV){
			...
			if (microdomains == "yes"){
				...
				// foci_distance_min, foci_distance_max, foci_distance_mean, foci_distance_stdDev, foci_distance_CV
//				foci_distribution = charaterize_foci_distribution(count_foci_from_intensity_profile_Gauss[0]);
...
*/
			...
			cell_res = parents[0] +","+ parents[1] // [0] - experiment code, [1] - biological replicate date
				...
				// in the following three lines: [0] - area, [1] - integral_intensity, [2] - mean_intensity, [3] - SD, [4] - CV
				...
				+","+ plasma_membrane_DIV_cytosol +","+ plasma_membrane_DIV_cell_integral +","+ cytosol_DIV_cell_integral;
...
//				+","+ protein_fraction_in_foci +","+ foci_distribution[0] +","+ foci_distribution[1] +","+ foci_distribution[2] +","+ foci_distribution[3] +","+ foci_distribution[4] //foci_distance_min, foci_distance_max, foci_distance_mean, foci_distance_stdDev, foci_distance_CV
...
function analyze_tangential(){
	...
	init = 0; // first ROI
	numROIs = roiManager("count"); // number of ROIs in the manager
	// a shortened loop that can be used for testing; activated in the beginning of the script by assigning: test = 1; only last 4(5?) ROIs are analyzed
	if (test == 1)
		init = numROIs - 4;
	// for each cell (ROI) measure (all reported intensities are corrected for background):
	// - area, intensity (mean + integral), SD and CV of mean intensity (same as for transversal images) 
	...
	for(j = init; j < numROIs; j++){
		...
		if (cell[0] > cell_size_min && cell[0] < cell_size_max){ // continue if the cell size falls between the lower and upper limit
...
//			run("Enlarge...", "enlarge=-" + Delta);
			...
			if (cell[4] > CV_threshold) // [4] - cell intensity CV; if the CV is not greater than CV_threshold (set to 0.3 by default), the cells are deemed to have no microdomains
...
//				run("Enlarge...", "enlarge=-"+Delta);
				...
				if (mask_mean > 0){
					...
					foci = measure_area_selection(); // [0] - area, [1] - integral_intensity_background, [2] - mean_intensity_background, [3] - SD, [4] - CV
					foci_area_fraction = 100*foci[0]/cell[0];
					protein_in_foci = 100*foci[1]/cell[1]; // ratio of integral intensities: foci/cell
				}
			...
			cell_res = parents[0] +","+ parents[1]
				...
				+","+ cell[0] +","+ cell[1] +","+ cell[2] +","+ cell[3] +","+ cell[4] // [0] - area, [1] - integral_intensity, [2] - mean_intensity, [3] - SD, [4] - CV
				...
				+","+ protein_in_foci; // ratio of integral intensities: foci/cell
			print("["+temp_file+"]", cell_res + "\n");
		}
	}
	close("*");
...
// folder with a name that starts with the accession code, containing subfolders, each starting with the date in the YYMMDD (ideally) format
// each biological replicate folder contains the "transversal" and/or "tangential folders
// each of these contains "data", "ROIs" and other folders
...
function print_header(){
	...
	if (matches(image_type, "transversal")){
		column_names = column_names
			...
			+ ",plasma_membrane_I.div.Cyt_I(mean),plasma_membrane_I.div.cell_I(integral),Cyt_I.div.cell_I(integral)";
		...
		if (microdomains == "yes")
			column_names = column_names
				+ ",foci,foci_density,foci_intensity,plasma_membrane_base,foci_prominence,foci_outliers"
//				+ ",prot_in_foci,foci_distance_min,foci_distance_max,foci_distance_mean,foci_distance_stdDev,foci_distance_CV"
...
*/
	...
	// in this case, microdomains are always quantified
		...
		column_names = column_names
			...
			+ ",length[um],length_SD[um],width[um],width_SD[um],size[um],size_SD[um]"
...
// analyze distribution of fl. maxima along the plasma membrane: shortest, longest and average distance, coeffcient of variance of their distribution (a measure of uniformity)
...
function charaterize_foci_distribution(foci){
	foci_distance_min = NaN;
	foci_distance_max = NaN;
	foci_distance_mean = NaN;
	foci_distance_stdDev = NaN;
	foci_distance_CV = NaN;
	if (foci > 1){
...
//		for (p = foci_outliers; p < foci; p++){ // at this point, maxima are ordered by intensity; starting at "foci_outliers" makes the algorithm discart P-bodies
			MAXIMA[p] = getResult("X1", p); // maxima are ordered by intensity in the results table, in descending order; here their positions are recorded into an array
		}
		Array.sort(MAXIMA); // sort positions of the intensity maxima in ascending manner
		...
		foci_distance = newArray(MAXIMA.length - 1);
		for (p = 0; p < MAXIMA.length - 1; p++){
		 	foci_distance[p] = MAXIMA[p+1] - MAXIMA[p];
		}
		Array.getStatistics(foci_distance, foci_distance_min, foci_distance_max, foci_distance_mean, foci_distance_stdDev);
		foci_distance_CV = foci_distance_stdDev/foci_distance_mean;
	}
	return newArray(foci_distance_min, foci_distance_max, foci_distance_mean, foci_distance_stdDev, foci_distance_CV);
}
...
// area, intensity (mean + integral), standard deviation, coefficient of variance; fit with an ellipse and measure both axes and its eccentricity
function measure_area_selection(){
	...
	major_axis = getResult("Major", 0); // the ROI is fitted with an ellipse and its parameters are measured
	minor_axis = getResult("Minor", 0);
	eccentricity = sqrt(1-pow(minor_axis/major_axis, 2)); // according to the standard formula
	return newArray(area, integral_intensity_background, mean_intensity_background, SD, CV, major_axis, minor_axis, eccentricity);
}
